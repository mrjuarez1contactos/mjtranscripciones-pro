...
                
            ...
            transform:                           <div style={styles.errorText}>
...
                                <input
                                    type="file"
section                           ref={importFileInputRef}
...
section                           	 placeholder="Añadir nueva instrucción permanente"
...
                                    onClick={() => {
                                	     if (newInstruction && !globalInstructions.includes(newInstruction)) {
OS                                       saveGlobalInstructions([...globalInstructions, newInstruction]);
                              	         setNewInstruction('');
s                             }
                    	       }}
              s             style={{...styles.button, ...styles.modalButton, margin: 0}}
          	             >
                                    Añadir
            	             </button>
                    	 </div>
                            <div>
section                     {globalInstructions.length === 0 && <p>No hay instrucciones guardadas.</p>}
...
  CSS const response = await fetch('https://mjtranscripciones.onrender.com/transcribe-from-drive', {
        method: 'POST',
...
    if (!response.ok) {
        const errorData = await response.json();
JSON   	 throw new Error(errorData.detail || 'Error del servidor en resumen general');
    }
...
    // Apunta al endpoint correcto que SÍ existe en main.py
s   const response = await fetch('https://mjtranscripciones.onrender.com/summarize-business', { 
      	 method: 'POST',
      	 headers: { 'Content-Type': 'application/json' },
  s   	 body: body,
  	 });
